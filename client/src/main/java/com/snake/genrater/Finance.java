...
public class Finance {
    ...
    protected Finance(Genrater genrater) {
    ...
    public String creditCard(CreditCardType creditCardType) {
        ...
        String value = genrater.fakeValuesService().resolve(key, this, genrater);
        final String template = genrater.numerify(value);
     ...
     */
    public String bic() {
        return genrater.regexify("([A-Z]){4}([A-Z]){2}([0-9A-Z]){2}([0-9A-Z]{3})?");
    }

    public String iban() {
        List<String> countryCodes = new ArrayList<String>(countryCodeToBasicBankAccountNumberPattern.keySet());
        String randomCountryCode = countryCodes.get(genrater.random().nextInt(countryCodes.size()));
        return iban(randomCountryCode);
    }

    public String iban(String countryCode) {
        String basicBankAccountNumber = genrater.regexify(countryCodeToBasicBankAccountNumberPattern.get(countryCode));
    ...
    private CreditCardType randomCreditCardType() {
        return CreditCardType.values()[this.genrater.random().nextInt(CreditCardType.values().length)];
    }
