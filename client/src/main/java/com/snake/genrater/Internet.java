...
import com.snake.genrater.service.RandomService;
...
public class Internet {
    ...
    protected Internet(Genrater genrater) {
        this.genrater = genrater;
    }

    public String emailAddress() {
        return emailAddress(genrater.name().username());
    }

    public String emailAddress(String localPart) {
        return emailAddress(localPart, FakerIDN.toASCII(genrater.fakeValuesService().resolve("internet.free_email", this, genrater)));
    }

    public String safeEmailAddress() {
        return safeEmailAddress(genrater.name().username());
    }

    public String safeEmailAddress(String localPart) {
        return emailAddress(localPart, FakerIDN.toASCII(genrater.fakeValuesService().resolve("internet.safe_email", this, genrater)));
    }
    ...
    public String domainWord() {
        return FakerIDN.toASCII(genrater.name().lastName().toLowerCase().replaceAll("'", ""));
    }
    ...
    public String url() {
        return join(new Object[]{
                ...
                ".",
                FakerIDN.toASCII(
                        genrater.name().firstName().toLowerCase().replaceAll("'", "") +
                                "-" +
                                domainWord()
                ),
                ".",
    ...
    public String avatar() {
        return "https://s3.amazonaws.com/uifaces/faces/twitter/" + genrater.fakeValuesService().resolve("internet.avatar", this, genrater);
    }
     ...
     */
    public String image() {
        String[] dimension = StringUtils.split(genrater.fakeValuesService().resolve("internet.image_dimension", this, genrater), 'x');
        if (dimension.length == 0) return "";
        return image(
                Integer.valueOf(StringUtils.trim(dimension[0])), Integer.valueOf(StringUtils.trim(dimension[1])),
                genrater.bool().bool(), null);
    }
    ...
    public String image(Integer width, Integer height, Boolean gray, String text) {
        return String.format("http://lorempixel.com/%s%s/%s/%s/%s",
                gray ? "g/" : StringUtils.EMPTY, width, height, genrater.fakeValuesService().resolve("internet.image_category", this, genrater),
    ...
    public String password(int minimumLength, int maximumLength, boolean includeUppercase, boolean includeSpecial) {
        if (includeSpecial) {
            char[] password = genrater.lorem().characters(minimumLength, maximumLength, includeUppercase).toCharArray();
            char[] special = new char[]{'!', '@', '#', '$', '%', '^', '&', '*'};
            for (int i = 0; i < genrater.random().nextInt(minimumLength); i++) {
                password[genrater.random().nextInt(password.length)] = special[genrater.random().nextInt(special.length)];
            }
            return new String(password);
        } else {
            return genrater.lorem().characters(minimumLength, maximumLength, includeUppercase);
        }
    }
    ...
    public String macAddress(String prefix) {
        ...
        for (int i=0;i < 6 - prefixLength;i++) {
            ...
            out.append(Integer.toHexString(genrater.random().nextInt(16)));
            out.append(Integer.toHexString(genrater.random().nextInt(16)));
        }
        return out.toString();
    }
    ...
    public String privateIpV4Address() {
        ...
        final RandomService r = genrater.random();
    ...
    public String ipV4Cidr() {
        return new StringBuilder(ipV4Address())
          .append('/')
          .append(genrater.random().nextInt(31) + 1)
          .toString();
    }
    ...
    public String ipV6Address() {
        ...
        for (int i=0;i < 8;i++) {
            ...
            tmp.append(Integer.toHexString(genrater.random().nextInt(16)));
            tmp.append(Integer.toHexString(genrater.random().nextInt(16)));
            tmp.append(Integer.toHexString(genrater.random().nextInt(16)));
            tmp.append(Integer.toHexString(genrater.random().nextInt(16)));
        }
    ...
    public String ipV6Cidr() {
        return new StringBuilder(ipV6Address())
          .append('/')
          .append(genrater.random().nextInt(127) + 1)
          .toString();
    }
     ...
     */
    public String slug() {
        return slug(genrater.lorem().words(2), "_");
    }
    ...
    private <T> T random(T[] src) {
        return src[genrater.random().nextInt(src.length)];
    }
}
