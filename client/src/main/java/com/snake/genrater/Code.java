...
public class Code {
    ...
    protected Code(Genrater genrater) {
    ...
    public String isbnGs1() {
        return genrater.regexify("978|979");
    }
    /**
     * This can be overridden by specifying
     * <code>
    ...
    public String isbnRegistrant() {
        int ct = genrater.random().nextInt(6) + 1;
        switch (ct) {
            case 6:
                return genrater.number().numberBetween(0,1) + genrater.number().digit() + "-" + genrater.number().digits(6);
            case 5:
                return genrater.number().numberBetween(200,699) + "-" + genrater.number().digits(5);
            case 4:
                return genrater.number().numberBetween(7000,8499) + "-" + genrater.number().digits(4);
            case 3:
                return genrater.number().numberBetween(85000,89999) + "-" + genrater.number().digits(3);
            case 2:
                return genrater.number().numberBetween(900000,949999) + "-" + genrater.number().digits(2);
            case 1:
                return genrater.number().numberBetween(9500000,9999999) + "-" + genrater.number().digits(1);
            default:
    ...
    public String isbn10(boolean separator) {
        ...
        final StringBuilder isbn10 = new StringBuilder()
            .append(genrater.expression("#{code.isbn_group}"))
            .append('-')
            .append(genrater.expression("#{code.isbn_registrant}"))
            .append('-');
    ...
    public String isbn13(boolean separator) {
        ...
        final StringBuilder isbn13 = new StringBuilder()
            .append(genrater.expression("#{code.isbn_gs1}"))
            .append('-')
            .append(genrater.expression("#{code.isbn_group}"))
            .append('-')
            .append(genrater.expression("#{code.isbn_registrant}"))
    ...
    public String asin() {
        return genrater.resolve("code.asin");
    }
    ...
    public String imei() {
        ...
        String arr = genrater.options().option(REPORTING_BODY_IDENTIFIERS);
        str[0] = arr.charAt(0);
        str[1] = arr.charAt(1);

        // Fill all the remaining numbers except for the last one with random values.
        for (int i=2; i < len - 1; i++) {
            str[i] = Character.forDigit(genrater.number().numberBetween(0, 9), 10);
        }
    ...
    public String gtin8() {
        char[] values = genrater.regexify("\\d{7}").toCharArray();
        int sum = 0;
    ...
    public String gtin13() {
        char[] values = genrater.regexify("\\d{12}").toCharArray();
        int sum = 0;
        for (int i = 0; i < values.length; i++) {
