...
class SequenceToSequence(object):
    ...
    def __init__(self, input_vocab_size, target_vocab_size, batch_size=32, embedding_size=300,
                 mode='train',
                 hidden_units=256,
                 depth=1,
                 beam_width=0,
        ...
        :param batch_size: 数据batch的大小
        :param embedding_size: 输入词表与输出词表embedding的维度
        :param mode: 取值为 train 或者 decode，训练模式或者预测模式
        :param hidden_units: RNN模型的中间层大小，encoder和decoder层相同, 如果encoder层是bidirectional的话，decoder层是双倍大小
        :param depth: encoder和decoder的rnn层数
        :param beam_width: beam_width是beamsearch的超参，用于解码,如果大于0则使用beamsearch，小于等于0则不使用
        ...
        :param max_decode_step: 最大的解码长度，可以是很大的整数，默认是None,None的情况下默认是encoder输入最大长度的 4 倍
        :param attention_type: 'Bahdanau' or 'Luong' 不同的 attention 类型
        :param bidirectional: encoder 是否为双向
        :param time_major: 是否在“计算过程”中使用时间为主的批量数据
        :param seed: 一些层间操作的随机数 seed 设置
        :param parallel_iterations: dynamic_rnn 和 dynamic_decode 的并行数量,如果要取得可重复结果，在有dropout的情况下，应该设置为
        :param share_embedding: 如果为True，那么encoder和decoder就会公用一个embedding
        ...
        self.rewards = None
        self.decoder_inputs_length = None
        self.decoder_start_token = None
        self.decoder_inputs_train = None
        ...
        assert 0.0 <= dropout < 1.0, '0 <= dropout < 1'
    ...
    def check_feeds(self, encoder_inputs, encoder_inputs_length,
        ...
        第一句的真实长度是 4，第二句只有 3（最后的</s>是一个填充数据）
        ...
        :param encoder_inputs: 一个整形二维矩阵 [batch_size, max_source_time_steps]
        :param encoder_inputs_length: 一个整形向量 [batch_size]，每个维度是encoder句子的真实长度
        :param decoder_inputs: 一个整形矩阵 [batch_size, max_target_time_steps]
        :param decoder_inputs_length: 一个整形向量 [batch_size]，每个维度是decoder句子的真实长度
        :param decode: 用来指示正在训练模式(decode=False)还是预测模式(decode=True)
        :return: tensorflow所操作需要的input_feed，包括encoder_inputs, encoder_inputs_length,decoder_inputs, decoder_inputs_length
        """
